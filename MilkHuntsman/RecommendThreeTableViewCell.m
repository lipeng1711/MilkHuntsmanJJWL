...
@implementation RecommendThreeTableViewCell
...
- (void)layoutSubviews
{
    ...
    for (int i = 0; i < self.firstThreeArray.count; i ++) {
        ...
        [self.recommendImageView setImageWithURL:[NSURL URLWithString:_firstThreeArray[2][i][@"title_page"]]];
        _recommendDescriptionLabel.text = _firstThreeArray[2][i][@"title"];
        _recommendMoneyLabel.text = [NSString stringWithFormat:@"¥%@",_firstThreeArray[2][i][@"price"]];
...
- (void)tapAction:(UITapGestureRecognizer *)sender
{
    ...
    [self.firstThreeDelegate firstThreeChangePageByIndext:indext];
