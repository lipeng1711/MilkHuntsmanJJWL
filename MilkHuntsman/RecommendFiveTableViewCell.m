...
@implementation RecommendFiveTableViewCell
...
- (void)layoutSubviews
{
    ...
    for (int i = 0; i < self.firstFiveArray.count; i ++) {
        ...
        [self.recommendImageView setImageWithURL:[NSURL URLWithString:_firstFiveArray[4][i][@"title_page"]]];
        _recommendDescriptionLabel.text = _firstFiveArray[4][i][@"title"];
        _recommendMoneyLabel.text = [NSString stringWithFormat:@"¥%@",_firstFiveArray[4][i][@"price"]];
...
- (void)tapAction:(UITapGestureRecognizer *)sender
{
    ...
    [self.firstFiveDelegate firstFiveChangePageByIndext:indext];
