...
@implementation RecommendTwoTableViewCell
...
- (void)layoutSubviews
{
    ...
    for (int i = 0; i < self.firstTwoArray.count; i ++) {
        ...
        [self.recommendImageView setImageWithURL:[NSURL URLWithString:_firstTwoArray[1][i][@"title_page"]]];
        _recommendDescriptionLabel.text = _firstTwoArray[1][i][@"title"];
        _recommendMoneyLabel.text = [NSString stringWithFormat:@"¥%@",_firstTwoArray[1][i][@"price"]];
...
- (void)tapAction:(UITapGestureRecognizer *)sender
{
    ...
    [self.firstTwoDelegate firstTwoChangePageByIndext:indext];
